...
# Hamiltonian - fixed delta and m, varying h
delta <- 0.1
m <- 5
n_iterations_per_value <- 100000

# h_vec <- c(0.01, 0.05, 0.1, 0.2, 0.25, 0.4, 0.5, 0.75, 1, 1.5, 2, 2.5)
h_vec <- c(seq(0.01, 0.09, 0.01), seq(0.1, 2.5, 0.1))
init_cluster <- parallel::makeCluster(8)
doParallel::registerDoParallel(init_cluster)
hamiltonian_results <- foreach::foreach(k = 1:length(h_vec), .combine = "rbind", .options.RNG = 123) %dorng% {
...
delta <- 0.05
rknf_results <- foreach::foreach(k = 1:length(h_vec), .combine = "rbind", .options.RNG = 123) %dorng% {
  reversibility_issue <- numeric(n_iterations_per_value)
  c_vec <- numeric(n_iterations_per_value)
  h <- h_vec[k]
  for (i in 1:n_iterations_per_value) {
...
delta <- 1
mrf_results <- foreach::foreach(k = 1:length(h_vec), .combine = "rbind", .options.RNG = 123) %dorng% {
  reversibility_issue <- numeric(n_iterations_per_value)
  c_vec <- numeric(n_iterations_per_value)
  h <- h_vec[k]
  for (i in 1:n_iterations_per_value) {
...
saveRDS(
  list_results, 
  "illustrations/comparison_prob_reversibility_and_coarsest_step/varying_h_results.RDS"
)

parallel::stopCluster(init_cluster)

list_results <- readRDS("illustrations/comparison_prob_reversibility_and_coarsest_step/varying_h_results.RDS")
...
par(mfrow = c(3, 2))
plot(h_vec, hamiltonian_results$prob_reversiblity_issue, type = "l", ylim = c(0, 0.175))
lines(h_vec, hamiltonian_results$lower_prob_reversibility_issue, lty = 2, col = "red")
lines(h_vec, hamiltonian_results$upper_prob_reversibility_issue, lty = 2, col = "red")
plot(h_vec, hamiltonian_results$prob_c_0, type = "l", ylim = c(0, 1))
lines(h_vec, hamiltonian_results$prob_c_1, col = "green")
lines(h_vec, hamiltonian_results$prob_c_2, col = "blue")
lines(h_vec, hamiltonian_results$prob_c_larger_2, col = "red")
plot(h_vec, rknf_results$prob_reversiblity_issue, type = "l", ylim = c(0, 0.175))
lines(h_vec, rknf_results$lower_prob_reversibility_issue, lty = 2, col = "red")
lines(h_vec, rknf_results$upper_prob_reversibility_issue, lty = 2, col = "red")
plot(h_vec, rknf_results$prob_c_0, type = "l", ylim = c(0, 1))
lines(h_vec, rknf_results$prob_c_1, col = "green")
lines(h_vec, rknf_results$prob_c_2, col = "blue")
lines(h_vec, rknf_results$prob_c_larger_2, col = "red")
plot(h_vec, mrf_results$prob_reversiblity_issue, type = "l", ylim = c(0, 0.175))
lines(h_vec, mrf_results$lower_prob_reversibility_issue, lty = 2, col = "red")
lines(h_vec, mrf_results$upper_prob_reversibility_issue, lty = 2, col = "red")
plot(h_vec, mrf_results$prob_c_0, type = "l", ylim = c(0, 1))
lines(h_vec, mrf_results$prob_c_1, col = "green")
lines(h_vec, mrf_results$prob_c_2, col = "blue")
lines(h_vec, mrf_results$prob_c_larger_2, col = "red")
par(mfrow = c(1, 1))
...
hamiltonian_results_df <- cbind(h_vec, hamiltonian_results)
...
rknf_results_df <- cbind(h_vec, rknf_results)
...
mrf_results_df <- cbind(h_vec, mrf_results)
