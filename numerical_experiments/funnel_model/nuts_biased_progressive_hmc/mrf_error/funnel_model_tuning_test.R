...
prob_no_step_size_halving <- 0.975
...
final_run <- foreach::foreach(i = 1:n_chains) %dopar% {
  ...
  dir.create("numerical_experiments/funnel_model/nuts_biased_progressive_hmc/mrf_error/log", showWarnings = F)
  sink(paste0("numerical_experiments/funnel_model/nuts_biased_progressive_hmc/mrf_error/log/log_nr_", i, ".txt"))
  print("Warmup")
...
saveRDS(final_run, "numerical_experiments/funnel_model/nuts_biased_progressive_hmc/mrf_error/funnel_model_tuning_test.RDS")
...
final_sampling_result <- # concatenate same-named elements together across the chains
  lapply(names(final_sampling_run[[1]]), function(element_name) {
    ...
    if (is.matrix(elements[[1]])) {
      if (element_name != "c_matrix" & element_name != "B_matrix") {
        do.call(rbind, elements)
      } else {
        max_ncol <- max(as.numeric(lapply(elements, ncol)))
        final_matrix <- function(matrix_obj) {
          current_ncol <- ncol(matrix_obj)
          if (max_ncol != current_ncol) {
            final_matrix_obj <- matrix(-1, ncol = max_ncol, nrow = nrow(matrix_obj))
            final_matrix_obj[, 1:current_ncol] <- matrix_obj
          } else {
            final_matrix_obj <- matrix_obj
          }
          final_matrix_obj
        }
        new_elements <- lapply(elements, final_matrix)
        if (element_name == "c_matrix") {
          as.matrix(
            dplyr::bind_rows(lapply(new_elements, as.data.frame))
          ) 
        } else {
          do.call(rbind, new_elements)
        }
      }
    } else {
      unlist(elements)
...
sink("numerical_experiments/funnel_model/nuts_biased_progressive_hmc/mrf_error/funnel_model_tuning_test.txt")
